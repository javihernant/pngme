...
impl Png {
    ...
    pub fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk> {
        if let Some(index) = self.chunks.iter().position(|x| x.chunk_type() == &ChunkType::from_str(chunk_type).unwrap()) {
            Ok(self.chunks.remove(index))
        }else {
            Err("No chunks were found with the chunk-type provided".into())
        }
    }
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(bytes : &[u8]) -> Result<Self> {
        let sig = bytes[..8].try_into().unwrap();
        ...
        let mut i = 8;
        while i < bytes.len() {
            let len_bytes = bytes[i..i+4].try_into().unwrap();
            let length = u32::from_be_bytes(len_bytes) + 12;
            if i + length as usize > bytes.len()
            {
                return Err("Corrupted data!".into());
            }
            chunks.push(Chunk::try_from(&bytes[i..][..length as usize]).unwrap());
            i += length as usize;
        }
